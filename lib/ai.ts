...
function isGatewayOffline(err: unknown): boolean {
...
export async function parseIntent(userMessage: string): Promise<TravelIntent> {
  ...
  try {
    const gw = new OpenAI({ apiKey: GATEWAY_API_KEY, baseURL: GATEWAY_URL })
    ...
    if (!isGatewayOffline(err)) throw err
    // Gateway offline — fall through to direct provider
  }
  ...
  return new ReadableStream({
    async start(controller) {
      try {
        ...
        try {
          const gw = new OpenAI({ apiKey: GATEWAY_API_KEY, baseURL: GATEWAY_URL })
          stream = await gw.chat.completions.create({
            model: 'auto',
            messages: [
        ...
        if (gatewayFailed && provider === 'qwen') {
          const qwen = new OpenAI({
            apiKey: process.env.DASHSCOPE_API_KEY,
            baseURL: 'https://dashscope.aliyuncs.com/compatible-mode/v1',
          })
